          ...
          <nav className="flex flex-col gap-1">
            {navItems.map((item) =>
              item.enabled ? (
                ...
                <div
                  key={item.href}
                  className="flex items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-sm text-[#C0C7D6] cursor-not-allowed"
                >
                  <div className="flex items-center gap-3">
                    {item.icon}
                    {item.label}
                  </div>
                  <span className="text-[10px] bg-[#E8EBF7] text-[#7C86B8] px-1.5 py-0.5 rounded-full font-medium">
                    Pronto
                  </span>
                </div>
              )
